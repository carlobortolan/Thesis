...
\begin{enumerate}
    ...
    \texttt{rtmp://rtmp-proxy.example.com/live/ <personal-token>}.
...
\begin{figure}[htpb]
    ...
    \caption[RTMP-Proxy Flow]{RTMP-Proxy Data Flow}\label{fig:rtmp-token}
\end{figure}
...
A better solution was to use the \href{https://github.com/arut/nginx-rtmp-module}{nginx-rtmp-module}. This module extends the Nginx to handle \ac{RTMP} streaming requests natively, making it a more stable solution~\parencite{nginx_rtmp_module}. 
Now, whenever a stream request arrives at the proxy server, Nginx intercepts the request and invokes a Lua script that extracts the stream key (the path token) and requests the actual destination URL from the GoCast \ac{API}. Upon receiving the destination URL, Nginx dynamically pushes the stream to the target server.
