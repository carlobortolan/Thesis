...
file_path = './data/Errors-data-2024-08-11 23_48_43.csv'
...
extracted_logs = new_df['Line'].str.extract(r'level=(\w+) msg="([^"]+)"')
...
# Analyze error logs over time by counting occurrences per hour
...
# Hourly Error Rate Analysis: Normalize errors over total logs to show the rate
total_logs_per_hour = new_df.resample('D').size()
error_rate = (error_logs_over_time / total_logs_per_hour).fillna(0) * 100  # Convert to percentage

# Trend analysis using rolling average
error_trend = error_logs_over_time.rolling(window=6).mean()  # 6-hour rolling average

# Analyze errors by hour
new_df['Hour'] = new_df.index.hour
error_logs_by_hour = new_df[new_df['level'] == 'error'].groupby('Hour').size()
...
for error_type in error_types.unique():
    ...
    error_logs_over_time = error_logs_filtered.resample('h').size()
    fig.add_trace(go.Scatter(
        x=error_logs_over_time.index, 
        y=error_logs_over_time.values, 
        mode='lines+markers', 
...
fig = go.Figure()
fig.add_trace(go.Bar(
    x=error_logs_by_hour.index, 
    y=error_logs_by_hour.values, 
    name='Errors by Hour',
    marker_color='crimson'
))
fig.update_layout(
    # title=='Distribution of Errors by Hour',
...
# Display results for user interaction
error_logs_by_hour
